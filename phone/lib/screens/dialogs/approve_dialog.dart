...
/// Dialog for approving an inbox item with optional note and feedback chips.
///
...
/// Usage:
/// ```dart
/// final feedback = await ApproveDialog.show(context, availableChips: chips);
/// if (feedback != null) { /* proceed */ }
/// ```
class ApproveDialog extends StatefulWidget {
  final List<String> availableChips;

  const ApproveDialog({super.key, required this.availableChips});
  ...
  static Future<ApproveFeedback?> show(
    ...
    return showDialog<ApproveFeedback>(
      context: context,
      builder: (ctx) => ApproveDialog(availableChips: availableChips),
    );
  }
